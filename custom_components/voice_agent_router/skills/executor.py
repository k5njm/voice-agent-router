...
from .cache import SkillResponseCache
...
class SkillExecutor:
    ...
    def __init__(
        ...
        response_cache: SkillResponseCache | None = None,
    ) -> None:
        self._hass = hass
    ...
    async def execute_template_skill(self, skill: SkillDefinition) -> str:
        ...
        if self._response_cache is not None:
            cached = self._response_cache.get(skill.name)
            if cached is not None:
                _LOGGER.debug("Cache hit for skill '%s'", skill.name)
                return cached
        ...
        for entity_pattern in skill.entities:
            try:
                if "*" in entity_pattern:
                    ...
                    for state in self._entity_cache.get_entities_by_domain(domain):
                        entity_states[state.entity_id] = state
                else:
                    state = self._entity_cache.get_entity_state(entity_pattern)
        ...
        try:
            ...
            # Store in cache if skill has cache config
            if (
                self._response_cache is not None
                and skill.cache_cron is not None
                and skill.cache_ttl > 0
            ):
                self._response_cache.put(skill.name, result, skill.cache_ttl)
            return result
